...
public sealed partial class SiteSettingsPanel : IDialogContentComponent<GlobalState>, IAsyncDisposable
{
    ...
    [Inject] BaseLayerLuminance BaseLayerLuminance { get; set; } = default!;
    [Inject] AccentBaseColor AccentBaseColor { get; set; } = default!;
    [Inject] Direction Direction { get; set; } = default!;
    ...
    protected override void OnInitialized()
    {
        ...
        _selectedColorOption = colors.FirstOrDefault(x => x.ToAttributeValue() == Content.Color);

        if (_selectedColorOption == OfficeColor.Default)
        {
            _selectedColorOption = colors[new System.Random().Next(colors.Length)];
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            _jsModule = await JSRuntime.InvokeAsync<IJSObjectReference>("import", "./_content/Ling.Tools.Wasm/js/theme.js");
